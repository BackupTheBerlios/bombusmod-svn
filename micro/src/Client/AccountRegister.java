            ...
            Runnable
{
    ...
    private Command cmdCancel=new Command(SR.MS_CANCEL,Command.BACK, 2); //locale
    ...
    public AccountRegister(Account account, Display display, Displayable parentView) {
        ...
        raccount=account;
        spl.setProgress(SR.MS_STARTUP,5); //locale
    ...
    public void run() {
        try {
            spl.setProgress("Connect to " +raccount.getServer(),30); //locale
    ...
    public void beginConversation(String SessionId) {
        spl.setProgress("Registering",60); //locale
    ...
    public void blockArrived( JabberDataBlock data ) {
        ...
        if (data instanceof Iq) {
            ...
            String title="Done";  //locale
            if (!type.equals("result")) {
                pgs=0;
                title="Error: " +((JabberDataBlock) //locale
